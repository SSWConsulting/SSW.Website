...
const TermsAndConditionsLayout: FC<{ data: termsAndConditionsProps }> = ({
	data,
}) => {
	...
	return (
		<>
			...
			<Section className="mx-auto w-full max-w-9xl py-5 px-8 text-xs">
				<TinaMarkdown content={_body} />
			</Section>
			<Section className="mx-auto w-full max-w-9xl flex-col px-8">
				<div className="pt-3 font-sans text-xl">{rates.title}</div>
				<table className="my-4 border-1 border-gray-300 text-xs leading-normal">
					...
					<tbody>
						{rates.rateList.map((item) => {
							return (
								<tr className="border-b-1 border-gray-300 even:bg-gray-75">
									<th
										scope="row"
										data-label="Resource"
									...
									<td
										data-label="Standard Hourly Rate"
										className="p-2 text-center"
									>
										{item.standardHourlyRate.AUD} +{" "}
										{item.standardHourlyRate.GST}
									</td>
									<td
										data-label="Standard Daily Rate"
										className="p-2 text-center"
									>
										{item.standardDailyRate.AUD} + {item.standardDailyRate.GST}
									</td>
									<td
										data-label="Prepaid Hourly Rate"
										className="p-2 text-center"
									>
										{item.prepaidHourlyRate.AUD} + {item.prepaidHourlyRate.GST}
									</td>
									<td
										data-label="Prepaid Daily Rate"
										className="p-2 text-center"
									>
										<strong>{item.prepaidDailyRate.AUD}</strong> +{" "}
										{item.prepaidDailyRate.GST}
									</td>
								</tr>
							);
						})}
				...
				<div
					className="mb-2.5 w-fit cursor-pointer border-b-1 border-dotted border-gray-450 text-gray-900"
			...
			<Section className="mx-auto w-full max-w-9xl py-5 px-8 text-xs">
				<div className="columns-2">
					<TinaMarkdown content={terms.content} />
				</div>
			</Section>
		</>
	);
