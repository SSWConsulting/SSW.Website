...
class MobileMenu extends React.Component {
  ...
  closeOpenedElements(element) {
    var openedItems = document.getElementsByClassName(
      cs(styles.dropdown, styles.open)
    );
  ...
  openItem(event) {
    if (event.target.parentNode.className === styles.dropdown) {
      ...
      event.target.parentNode.parentNode.className === styles.dropdown
    ) {
      this.closeOpenedElements(event.target.parentNode);
      this.openElement(event.target.parentNode.parentNode);
    } else if (
      event.target.parentNode.className === cs(styles.dropdown, styles.open)
    ) {
